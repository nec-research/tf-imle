...
def to_cmd(c, _path=None):
    i_str = '_inverse' if c["inverse_scaling"] is True else ''
    command = f'PYTHONPATH=. python3 ./cli/warcraft-cli.py settings/warcraft_shortest_path/12x12_map.json  ' \
                      ...
                      f'trainer_params.mode.scale_temperature_by{i_str}_path_length=True ' \
...
def main(argv):
    hyp_space = dict(
        ...
        nb_samples=[1, 10, 100],
        temperature=[1.0, 10.0, 100.0, 1000.0, 10000.0],
        inverse_scaling=[True, False],
        use_gamma=[True, False]
    )

    configurations = list(cartesian_product(hyp_space))

    path = 'logs/warcraft/temperature_beaker_v2'
    is_rc = False
