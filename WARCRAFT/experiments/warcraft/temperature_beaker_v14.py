...
def to_cmd(c, _path=None):
    maximize_str = 'True' if c["maximize"] is True else 'False'
    command = f'PYTHONPATH=. python3 ./cli/warcraft-cli.py settings/warcraft_shortest_path/12x12_map.json  ' \
              ...
              f'trainer_params.mode.dpo.maximize={maximize_str} ' \
...
def to_logfile(c, path):
    outfile = "{}/temperature_beaker_v14.{}.log".format(path, summary(c).replace("/", "_"))
    return outfile
...
def main(argv):
    hyp_space = dict(
        mode_type=[5],
        loss_type=['fy'],
        # objective_type=['normal', 'dpo'],
        objective_type=['dpo'],
        # maximize=[True, False],
        maximize=[False],
        nb_samples=[1],
        lam=[0.0],
        temperature=[0.0, 0.01, 0.1, 0.5, 1.0, 5.0, 10.0],
        use_gamma=[False],
        perturb_w=[False],
    ...
    path = 'logs/warcraft/temperature_beaker_v14'
    is_rc = False
