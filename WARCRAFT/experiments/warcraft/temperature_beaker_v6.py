...
def to_cmd(c, _path=None):
    command = f'PYTHONPATH=. python3 ./cli/warcraft-cli.py settings/warcraft_shortest_path/12x12_map.json  ' \
              ...
              f'loader_params.use_test_set=false'
    return command


def to_logfile(c, path):
    outfile = "{}/temperature_beaker_v6.{}.log".format(path, summary(c).replace("/", "_"))
    return outfile
...
def main(argv):
    hyp_space = dict(
        ...
        temperature=[0.001, 0.01, 0.1, 0.2, 0.5, 1.0],
        use_gamma=[True],
    ...
    path = 'logs/warcraft/temperature_beaker_v6'
    is_rc = False
