...
def to_cmd(c, _path=None):
    command = f'PYTHONPATH=. python3 ./cli/energy-cost-cli.py -b {c["batch_size"]} -l {c["learning_rate"]} ' \
              f'-e {c["nb_epochs"]} -m {c["mode"]} -L {c["lbda"]} -w {c["l2"]} -n {c["num_layers"]}'
    return command


def to_logfile(c, path):
    outfile = "{}/energy_parallel_v4.{}.log".format(path, summary(c).replace("/", "_"))
    return outfile
...
def main(argv):
    ...
    path = 'logs/energy/energy_parallel_v4'
    is_rc = False
